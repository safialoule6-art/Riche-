...
function showWelcomeBack(session){
  const run = async ()=>{
    ...
    // On affiche le PRÉNOM seulement s'il a été EXPLICITEMENT saisi (onboarding),
    // sinon l'email du compte. On ne déduit JAMAIS un prénom du profil Google
    // (source arbitraire — cf. bug "Leister") :
    //   1) localStorage sunami-firstname (saisi par l'apprenant sur cet appareil)
    //   2) cloud progress.first_name (saisi puis persisté)
    //   3) sinon → email (identifiant du compte, pas un prénom inventé)
    if(emailEl && session && session.user){
      const email = session.user.email || '';
      const clean = (v)=>{
        v = String(v || '').trim();
        if(!v || v.includes('@') || v.length > 24) return '';
        return v;
      };
      // 1) prénom explicitement saisi à l'onboarding
      let name = '';
      try{ name = clean(localStorage.getItem('sunami-firstname')); }catch(_){}
      emailEl.textContent = name || email; // prénom saisi, sinon email
      // 2) cloud (asynchrone) : prénom saisi et persisté, prioritaire s'il existe
